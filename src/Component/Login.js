  ...
  const loginData = async () => {
    ...
    try {
      const result = await axios.post(
        "http://localhost:5000/leavemanagementapi/dologin",
        logData
      );
      ...
      if (result.data.data.UserDesignation == "Employee") {
        navigator("/emp");
      } else if (result.data.data.UserDesignation == "HR") {
        navigator("/hr");
      } else {
        navigator("/manager");
      }
  ...
  return (
    <div className="login-Container" style={{ marginTop: "80px" }}>
      <div className="login-Form">
        ...
        <button className="login-btn" onClick={() => loginData()}>
          <span>Login</span>
        </button>
      </div>
    </div>
